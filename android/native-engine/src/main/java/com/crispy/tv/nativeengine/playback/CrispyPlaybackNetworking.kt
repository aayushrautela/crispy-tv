...
/**
 * Network stack for the ExoPlayer path. Uses a tuned OkHttpClient (browser
 * User-Agent, 15s timeouts, retryOnConnectionFailure, IPv4-first DNS) so the
 * Auto engine's Exo path survives re-buffers and resumes at the correct offset
 * instead of restarting from 0.
 */
internal object CrispyPlaybackNetworking {
    ...
    fun createHttpDataSourceFactory(
        ...
        val client = requestHeaders["Authorization"]?.let { authorization ->
        ...
        return OkHttpDataSource.Factory(client).apply {
            ...
            if (requestHeaders.none { it.key.equals("User-Agent", ignoreCase = true) }) {
    ...
    private fun withDefaultUserAgent(headers: Map<String, String>): Map<String, String> {
        ...
        if (sanitized["User-Agent"] != null) return sanitized

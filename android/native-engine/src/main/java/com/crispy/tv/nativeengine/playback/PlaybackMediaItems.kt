...
private fun inferMimeTypeFromPath(path: String?): String? {
    ...
    return when {
        ...
        extension == "mpeg" || extension == "mpg" -> MimeTypes.VIDEO_MPEG
            ...
            inferMimeTypeFromQuery(queryPart)
...
/**
 * Performs an HTTP HEAD then GET probe to read the response Content-Type.
 * Non-http(s) URLs return null. Timeouts are short; failures swallow to null.
 * Runs on the caller's thread — call from a background dispatcher.
 */
internal fun probeMimeType(
    url: String,

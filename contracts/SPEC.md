...
- `watch_sync`
  ...
  - Client policy: open the stream when a consuming surface is visible/foreground; close it when hidden/backgrounded. On `connection_opened` (reconnect) emit `refetch_continue_watching` only — home catch-up is served by the generations gate (below), never by a blind reconnect refetch. On any `watch_changed` while connected emit the per-`kind` effects. The server DB/cache is the source of truth; the stream is an invalidation trigger only.
  - Generations gate: `GET /v1/profiles/:profileId/watch/generations` returns per-surface epoch markers (`continue_watching`, `history`, `watchlist`, `ratings`, `home`) stamped when a change is published (DB-synchronous for write surfaces, rebuild-complete for `home`). Clients compare against the last locally-applied marker once per surface-open; a newer marker means cached data is stale and that surface must refetch. This covers changes that happened while the app was closed or offline (a missed signal), with one tiny read instead of full collection fetches. On generations read failure fall back to current fetch-on-gap behavior rather than showing stale cache.
  - `contract_version` 2: adds the five-kind `kind` routing, `refetch_history`/`refetch_watchlist`/`refetch_ratings`/`refetch_home` effects, and the `refetch_home` side effect on `connection_opened`. `contract_version` 3 removes the `connection_opened → refetch_home` side effect (revert to `refetch_continue_watching` only); home freshness is owned by the generations gate.
